...
## Decision Log
- [2026-09-20] **is_uplink 定案（用户）：自动推导**，两条信号各管一段场景 —— 多层站点用 **STP 根端口**（非核心交换机朝根桥的口），单机站点用**对端是 SD-WAN 的 LAN 口**（邻居类型/端口描述）。落地时我补了两条必要规则：① **刻意不把"对端是交换机"算作上行**（核心交换机朝下的口对端也是交换机，全算会把下行口误标）；② **聚合口展开**（STP 根端口常是 `Po1`/`lag49`，is_uplink 标在物理口上 → 不展开则十几台等于没标）。手工 `devices.uplink_ports` 降为"覆盖/佐证"。真机验证：**34/36 台推导出上行口**（原先手工字段仅 4 台）。

## Key Learnings
- [2026-09-20] **版本号方案 = `2 + 月份 + 日期`**（不是 semver！）：`2.9.20` = 2026-09-20，`2.8.31` = 8 月 31 日，`2.9.14` = 9 月 14 日 —— 这解释了历史上那些"奇怪的 patch 号"。发版时要同步 **5 处**：`VERSION`（权威，后端 `/api/version` 每次读文件，改完免重启）、`frontend/package.json`、`start.bat`、`README.md` 徽章、`NDM用户使用文档.html`（页头 + 页脚共 3 处）。我按 semver 猜成 2.10.0 被用户纠正过一次。
- [2026-09-20] **`.wolf/buglog.json` 的 `id` 不唯一** —— hook 自动生成的条目会与手工条目撞名（bug-151、bug-201 各撞过一次），**按 id 匹配会改错条目**（我因此犯了两次）。改 buglog 必须**按 error_message 内容匹配**，并顺手用 git 里的版本核对被误改的条目。
- [2026-09-20] **真机数据复核的价值（又一次）**：上行口推导实现完先跑真机 36 台，才发现两件事——① 字段名写错（`ConfigParser` 的条目字段是 `name` 不是 `interface`，会被 try/except 静默吞掉、退回手工清单）；② 聚合口没展开（15+ 台的根端口是 Po/pag）。**纯函数单测全绿也挡不住这两类问题**，因为它们是"接线/真实数据形态"的问题。
...
- **收尾项（2026-09-20 晚，用户要求）**：**全部完成** —— ① `is_uplink` 改为采集侧自动推导（STP 根端口 → 上游设备对端 → 描述关键词 → 手工覆盖 + LAG 展开；真机 34/36 台）；② **CLAUDE.md 架构段更正**（原描述的 cli_main.py / lib/collector.py / utils/storage.py / 根级 analyzers 均不存在，按真实结构重写）；③ 删除 `backend/tests/_main.py`（陈旧 main 副本）；④ 删除前端 `deviceApi.batchCollect`（死代码，指向不存在的 `/collect/batch`）；⑤ 删除仓库根目录遗留的 `tests/`（首次提交的规划稿，曾两次捕获相对路径 → bug-201）。提交链 e640f4c → 5355912 + 测试补充；测试 **478 项全绿**。
- **待办（外部依赖，用户知悉）**：① Cisco EoX 凭据**短期办不下来** → 型号 EoL 走手工登记（页面已支持），凭据到位后点刷新即自动；② SNTC 订阅待确认（决定保修能否自动化）；③ **CUI/CMMC/等保 待用户向合规口确认** —— 简报会把审计条目（已打码凭据、不含配置原文）发给第三方 LLM，若涉及 CUI 需重新评估；④ 生命周期数据待用户登记（Aruba 18 台 EoL + 各家保修，走批量导入）。
...
- **收尾（2026-09-21 下午，用户追加要求）**：① **fix 全面命令化**（62 规则：去中文说明、拆 `／` 与 `/` 为真换行、去行内注释、占位符统一中文；5 条流程类加 `# ` 前缀不执行）+ **未替换占位符警示**（`<设备名>` 字面下发会把主机名真改掉）；② 批量执行页**命令框右侧「设备当前配置（供参照）」**（by-rule 逐台带现状，按文本去重合并 `现状 ← 设备…`）。规则改动后**重跑审计**（run 22，477 条，ruleset_hash 已变）。测试 **550 项全绿**；浏览器实测通过（左 `no ntp server <公网地址>` + 右 `ntp server pool.ntp.org … ← 5 台`）。提交链 a0562b1 → d21aa23 → 89e921c → b097d6f。
- **发布 2.9.21**（同日）：版本号五处同步（VERSION/start.bat/package.json/README 徽章/使用文档）+ README 补「批量执行」「发现汇总视图」（功能特性/使用章节/结构树/存储表）。提交 df89a37 + fa86ad3。
- **待办**：《NDM用户使用文档》的「批量执行」章节未补（只同步了版本号）——用户需要时按 `F:\temp\add_stp_chapter.py` 的流程做（中英各一章 + 编号整段顺延 + 锚点断言）；README/使用文档的版本号方案见上文。
...
## Key Learnings（2026-09-21 补充：fix 命令化 + 现状参照）
- [2026-09-21] **规则库 fix 全面命令化**（用户要求"说明文字改为可直接执行的命令"）：去中文说明、把 `／`（或）与 `/`（多行）分隔统一为**真换行**、去行内 `! 注释`、省略号占位改中文占位符（`<认证口令>`）。**流程类**（改名/VLAN 迁移/生命周期）无命令可给 → fix 首行加 `# ` 前缀（`split_commands` 跳过 `#`，带入批量执行页天然不执行）。**占位符保留**（总部标准文档 CFG-CISCO/Aruba 自己也是占位符形态）—— 值靠右侧现状参照填。
...
- [2026-09-21] **设备名/命令输入框用正文普通字体，不要 monospace**（用户反馈"'设备'列的字体不好看，统一用'型号'列的字体"）——等宽只保留在代码/配置内容的展示处。新页面做表格时设备名列直接用正文体（0.72rem）。
- [2026-09-21] **全站字体统一规则**（用户要求"检查其他页面字体不统一"后逐页核查定案）：
  - **业务/资产数据**（设备名、型号、序列号、IP、位置、时间戳）→ **主题正文（IBM Plex Sans）**
  - **程序性内容**（配置文本、命令、日志原文、规则/例外 ID、版本号、端口名/速率）→ **`"Fira Code", monospace`**（唯一合法的等宽写法）
...
## User Preferences（2026-09-22 生命周期页评审定案）
- [2026-09-22] **编辑字段放编辑弹窗里，不要放页面工具栏**：生命周期页的「核实人」已从工具栏移进维保编辑弹窗（弹窗内记住本次会话上次填写的名字）。新页面照此办理——凡"保存时要填的字段"就放在保存它的弹窗里。
- [2026-09-22] **型号 EoL 保持"点型号单元格 → 型号级编辑"入口**（不改行内编辑、不加顶部型号区）：EoL 是**型号级**属性，一条公告覆盖所有同型号设备；用户明确接受该入口形态，但要求"改一次型号 → 所有同型号行同步更新 + 显示刷新"——已加测试钉死（`test_型号EoL改动同步所有同型号行`）+ 运行实例核对（26 型号中 14 个跨多行，日期与状态完全一致）。
- [2026-09-22] 教训（用户反馈触发）：把可点元素做成**无任何提示的纯文本按钮**，用户必然找不到（型号格的 EoL 入口）。用户选择保留现状；**以后做入口仍要显式化**（图标/下划线/tooltip 三选一）。
